...
export const setApiUrl = async (newUrl: string) => {
  // Ensure trailing slash is removed and /predict is correct
  ...
  API_URL = cleanUrl;
  try {
    await AsyncStorage.setItem(STORAGE_KEY, API_URL);
  } catch (e) {
...
export const initService = async () => {
  try {
    ...
    if (storedUrl) {
      API_URL = storedUrl;
      console.log("🔄 Loaded cached API URL:", API_URL);
    }
  } catch (e) {
...
const MOCK_DB: Record<string, ObjectData> = {
  lightbulb: {
    ...
    description:
      "A classic electric light with a wire filament heated until it glows.",
    components: [
      {
        id: "1",
        name: "Filament",
        description: "Tungsten wire that glows when heated.",
        position: { x: 50, y: 30 },
      },
      {
        id: "2",
        name: "Glass Bulb",
        description: "Protects the filament from oxidation.",
        position: { x: 50, y: 10 },
      },
      {
        id: "3",
        name: "Base",
        description: "Connects to the electrical supply.",
        position: { x: 50, y: 80 },
      },
    ],
  },
  mouse: {
    ...
    name: "Computer Mouse",
    description:
      "Hand-held pointing device that detects two-dimensional motion.",
    components: [
      {
        id: "1",
        name: "Scroll Wheel",
        description: "Used for scrolling content.",
        position: { x: 50, y: 20 },
      },
      {
        id: "2",
        name: "Left Click Switch",
        description: "Primary selection button.",
        position: { x: 30, y: 30 },
      },
      {
        id: "3",
        name: "Optical Sensor",
        description: "Detects movement relative to a surface.",
        position: { x: 50, y: 60 },
      },
    ],
  },
    ...
    components: [
      {
        id: "1",
        name: "SoC (Processor)",
        description: "The brain of the device.",
        position: { x: 50, y: 40 },
      },
      {
        id: "2",
        name: "Battery",
        description: "Provides power to the components.",
        position: { x: 50, y: 60 },
      },
      {
        id: "3",
        name: "Camera Module",
        description: "Captures photos and videos.",
        position: { x: 70, y: 15 },
      },
    ],
  },
  keyboard: {
    ...
    description:
      "Input device used to enter characters and functions into the computer system.",
    components: [
      {
        id: "1",
        name: "Key Switches",
        description: "Mechanisms under the keys that register presses.",
        position: { x: 50, y: 50 },
      },
      {
        id: "2",
        name: "Controller",
        description: "Scans the key matrix and sends data to the computer.",
        position: { x: 80, y: 20 },
      },
    ],
  },
  bottle: {
    ...
    name: "Water Bottle",
    description:
      "A container that is used to hold water, liquids or other beverages.",
    components: [
      {
        id: "1",
        name: "Cap",
        description: "Seals the bottle to prevent spilling.",
        position: { x: 50, y: 10 },
      },
      {
        id: "2",
        name: "Body",
        description: "Main container area.",
        position: { x: 50, y: 50 },
      },
    ],
  },
  cup: {
    ...
    name: "Coffee Mug",
    description:
      "A type of cup typically used for drinking hot beverages, such as coffee, hot chocolate, or tea.",
    components: [
      {
        id: "1",
        name: "Handle",
        description: "Used to hold the mug when hot.",
        position: { x: 80, y: 50 },
      },
      {
        id: "2",
        name: "Rim",
        description: "The edge where you drink from.",
        position: { x: 50, y: 10 },
      },
    ],
  },
  laptop: {
    ...
    components: [
      ...
      },
      {
        id: "2",
        name: "Motherboard",
        description: "Main circuit board with CPU and RAM.",
        position: { x: 50, y: 60 },
      },
      {
        id: "3",
    ...
    components: [
      {
        id: "1",
        name: "Movement",
        description: "Internal mechanism that keeps time.",
        position: { x: 50, y: 50 },
      },
      {
        id: "2",
        name: "Crystal",
        description: "Protective glass over the dial.",
        position: { x: 50, y: 30 },
      },
    ],
  },
...
const mapPredictionToObject = (
  ...
  for (const prediction of predictions) {
    ...
    if (
      (className.includes("phone") || className.includes("cellular")) &&
      MOCK_DB.phone
    )
      return MOCK_DB.phone;
    if (className.includes("keyboard") && MOCK_DB.keyboard)
      return MOCK_DB.keyboard;
    if (className.includes("bottle") && MOCK_DB.bottle) return MOCK_DB.bottle;
    if ((className.includes("cup") || className.includes("mug")) && MOCK_DB.cup)
      return MOCK_DB.cup;
    ...
    if (
      (className.includes("light") || className.includes("lamp")) &&
      MOCK_DB.lightbulb
    )
  ...
  return {
    ...
    name: displayName,
    description: `Identified as ${topPrediction.className} with ${(
